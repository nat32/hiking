...
<div class="container">
<h1>Randonnées effectuées</h1>

    <c:choose>
        <c:when test="${param.message==1}">
            <p>  La note a bien été modifiée  </p>
        </c:when>
        <c:when test="${param.message==2}">
            <p>  Il y a eu une erreur lors de la modification d'une note. Veuillez réessayer svp</p>
        </c:when>
        <c:when test="${param.message==3}">
            <p> La randonnée a bien été ajoutée aux randonnées effectuées</p>
        </c:when>
...
<table id="hikes" style="width:100%" class="display table table-striped table-responsive">
    ...
    <tr>
        ...
        <th align="center" scope="col">Ajouter/changer une note</th>
        <th align="center"  scope="col">Montrer</th>
        ...
        <tr>
            ...
            <td align="center">
                <c:if test="${hike.rating != null}">
                    <c:out value="${hike.rating}"/>
                <a type="button" data-id="${hike.id}"  data-userid="${hike.user_id}" class=" my-modal" data-toggle="modal" data-target="#myModal"><i class="far fa-edit"></i></a>
                </c:if>
            </td>

            <td align="center" ><a href="/hiking/showHike/${hike.id}"> <i class="fas fa-search"></i></a> </td>

          <td align="center"> <a href="/hiking/deleteHike/${hike.id}/${hike.user_id}/doneHikes" > <i class="far fa-trash-alt"></i></a>  </td>
        </tr>
...
<script>
    ...
    $('.my-modal').on('click', function () {
        ...
        $("div.modal-body a.btn-info").each(function() {
            ...
            current_link.attr("href", _href + rating +'/'+user_id + '/' +hike_id + '/doneHikes');
            rating++;
        });
    })
</script>
